        ...
        {list.map((item, i) => (
          <div key={item} className="flex  items-center gap-[10px]">
            <div className={`font-medium  leading-[22.5px] text-neutral-500`}>
              <span className={ list.length === i + 1 ? `text-[--primary]` : `text-neutral-500` }>
                {item}
              </span>
            ...
            {list.length >= i + 2 ? (
              <div
                className={`text-[20px] font-medium leading-[22.5px] ${list.length === i + 2 ? `text-[--primary]` : `text-neutral-500`}`}
              >{`<`}</div>
            ) : (
              ""
            )}
